...
int32_t getBtPlatfrom() {
  BLUETOOTH_TRACE("ADV %d", bluetooth.config.advertising_interval);
  BLUETOOTH_TRACE("BROADCAST %d", bluetooth.config.broadcast_interval);
...
void BluetoothLE::readline_(uint8_t * buffer, uint8_t length)
{
  uint8_t byte;
  int bufferIndex = 0;
  buffer[0] = '\0';
...
char * BluetoothLE::readline(bool error_reset)
{
    /*
    readline_(bt_data, BLUETOOTH_LE_PACKET_SIZE);
    if(bt_data[0]=='\0') return nullptr;
    else return bt_data;
    */
    return nullptr;
}
      ...
      if (crc == bt_data[13]) {
        if (bt_data[0] == 0x80) {
          ...
          for (uint8_t channel=0, i=1; channel<BLUETOOTH_TRAINER_CHANNELS; channel+=2, i+=3) {
            ...
            trainerInput[channel] = bt_data[i] + ((bt_data[i+1] & 0xf0) << 4) - 1500;
            trainerInput[channel+1] = ((bt_data[i+1] & 0x0f) << 4) + ((bt_data[i+2] & 0xf0) >> 4) + ((bt_data[i+2] & 0x0f) << 8) - 1500;
          }
          trainerResetTimer();
        }
      }
...
uint32_t BluetoothLE::send(uint8_t* frame, size_t cmd_size) {
  ...
  bluetoothWrite(frame,cmd_size);

  if (state == BLUETOOTH_STATE_CONNECTED) {
    return btle::transmit_time_ms(cmd_size, (btle::Baudrate)currentBaudrate, btle::DeviceType::ANDROID) / 2;
  }
  return WAIT_100MS;
}
...
uint32_t BluetoothLE::handleConfiguration() {
  ...
  uint8_t byte;
  rxIndex = 0;
  while (bluetoothRead(&byte) && rxIndex < sizeof(bt_data)) {
    bt_data[rxIndex++] = byte;
  }
...
void BluetoothLE::wakeup()
{
  ...
  if (currentBaudrate != g_eeGeneral.bluetoothBaudrate) {
    ...
    rxIndex = 0;
    wakeupTime = now + send(bt_data, btle::fw_version(bt_data));
    return;
  } 
  ...
  if (state == BLUETOOTH_STATE_CONNECTED) {
    ...
    return;
    //btle::transmit_time_ms(fifoSize, (btle::Baudrate)currentBaudrate, btle::DeviceType::ANDROID) / 2;
  }
  handleConfiguration();
  return;//
}
